...
public class Storage3 {
    ...
    public int [] numSeats;
    public int [] totalElectorate;
    public int [] totalPoll;


    public HashMap<String, Integer> seatsPerConstituency;
...
//1,34,0,0,0,0,2,1,0,0,0,4,0,27,0,0,0,0,0,0
    ...
    public HashMap<String, Integer> roundCandidateWon;
    // Constituency Name,Constituency Ainm,Count Number,Date Of Election,Number Of Candidates,Number of Seats,Quota,Required Save Deposit,Seats Filled,SeatsinConstit,Spoiled,Total Electorate,Total Poll,Valid Poll,Constituency Number
    // Count Number,Date Of Election,Number Of Candidates,Number of Seats,Quota,Required Save Deposit,Seats Filled,SeatsinConstit,Spoiled,Total Electorate,Total Poll,Valid Poll,Constituency Number
    public Storage3(Table table, Table table2) {
        N = table.getRowCount();
        ...
        ConstituencyName = new String[N2];
        countNumber = new int [N2];
        numSeats = new int [N2];
        totalElectorate = new int [N2];
        totalPoll = new int [N2];

        seatsPerConstituency = new HashMap<String, Integer>();

        ReadyTableTwo(table2);


    }

    public void ReadyTableTwo(Table table) {

        for (int i = 0; i < N2; i++) {
            TableRow row = table.getRow(i);

            ConstituencyName[i] = row.getString("ConstituencyName");
            countNumber[i] = row.getInt("CountNumber");
            numSeats[i] = row.getInt("NumberofSeats");
            totalElectorate[i] = row.getInt("TotalElectorate");
            totalPoll[i] = row.getInt("TotalPoll");

            seatsPerConstituency.put(ConstituencyName[i], numSeats[i]);
        }
    }

    public void ReadyTable(Table table) {
        roundCandidateWon = new HashMap<String, Integer>();
...
//    The National Party - An P�irt� N�isi�nta,United People,Workers Party
            ...
            try {
                this.roundCandidateWon.put(firstname[i] + " " + surname[i] + " " + id[i], round_won[i]);
            } catch (Exception e) {
                System.out.print(firstname[i] + " " + surname[i] + " " + id[i]);
            }
        ...
        adPartyColours = new HashMap<String, Colour>(){{
                ...
                //put("Fianna Fáil", new Colour(255, 255, 0)); // FF
